...
module.exports = async function tempmailCreate(
    ...
    try {
        await rizkyyy.sendMessage(m.chat, { react: { text: "⏱️", key: m.key } });

        let res = await fetch(`https://rynekoo-api.hf.space/tools/tempmail/v3/create`);
        if (!res.ok) return replyrizkyyy(`❌ Gagal menghubungi server.`);

        let json = await res.json();
        if (!json.success) return replyrizkyyy(`❌ Server Error.`);
...
Pilih aksi di bawah:` 
                        ...
                        footer: { text: "Rizky Official" },
                        header: { title: "🕹️ Fitur Temp Mail by Rizky" },
                        nativeFlowMessage: {
                            buttons: [{
                                name: "single_select",
        ...
        console.log(err);
        replyrizkyyy(`❌ Error: ${err.message}`);
    }
};
