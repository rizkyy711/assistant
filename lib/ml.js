...
module.exports = {
    ...
    run: async (rizkyyy, m, args) => {
        const text = args.join(" ")

        if (!text) {
            return rizkyyy.reply(m.chat, `Masukkan nama hero!\n\nContoh:\n.mlhero Nana`, m)
        }

        try {
            await rizkyyy.sendMessage(m.chat, { text: "🔍 Mencari informasi hero..." }, { quoted: m })

            const result = await mlHeroes(text)

            if (result.code !== 200) {
                return rizkyyy.reply(m.chat, result.message, m)
            }
...
`.trim()
            ...
            if (hero.image) {
                return rizkyyy.sendMessage(m.chat, {
                    image: { url: hero.image },
                    caption
                }, { quoted: m })
            } else {
                return rizkyyy.reply(m.chat, caption, m)
            }

        } catch (err) {
            console.error(err)
            return rizkyyy.reply(m.chat, "❌ Terjadi error, coba lagi nanti.", m)
        }
    }
}
