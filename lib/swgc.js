...
async function swgcHandler(m, rizkyyy, store, text, Ryuu, mess, replytolak) {
    let content = {};
    ...
    if (m.quoted) {
        try {
            ...
            if (!buffer) return rizkyyy.sendText(m.chat, "❌ Gagal mengambil media quoted.");
            ...
            if (/image|video|audio/.test(quotedType)) {
                if (/image/.test(quotedType)) {
                    ...
                    if (text) {
                        ...
                        return rizkyyy.sendText(m.chat, "Audio tidak boleh disertai caption.");
                    }
                ...
                fs.unlinkSync(tempFile);
                return rizkyyy.sendText(m.chat, "Reply harus berupa image/video/audio.");
            }
        } catch (e) {
            return rizkyyy.sendText(m.chat, "❌ Media tidak valid atau gagal diproses.");
        }
    } else if (text) {
        content.text = text;
    } else {
        return rizkyyy.sendText(m.chat, "Kirim media (foto/video/audio) atau teks, bisa dengan reply atau langsung.");
    }

    if (content.text && !content.text.trim()) 
        return rizkyyy.sendText(m.chat, "Teks tidak boleh kosong.");
    ...
    for (let gid of grupList) {
        try {
            let metadata = await rizkyyy.groupMetadata(gid);
            validGroups.push({
    ...
    if (!validGroups.length) {
        ...
        return rizkyyy.sendText(m.chat, "Tidak ada grup valid yang bisa dipilih.");
    }
    ...
    await rizkyyy.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
}

async function sendStatusHandler(m, args, rizkyyy, Ryuu, replytolak, groupStatus) {
    ...
    rizkyyy.sendText(m.chat, `Berhasil dikirim ke grup id: ${groupId}`);
}
