...
async function response(data, includeDL = false) {
    const response = {
        ...
        fsId: data.fs_id,
        message: data.message || 'Informasi tidak tersedia'
    };
...
const terabox = {
    detail: async function (link) {
        ...
        if (!id) throw new Error('⚠️ Masukkan tautan Terabox yang valid dan lengkap.');
        ...
        try {
            ...
            console.error(error);
            throw new Error('❌ Gagal memproses permintaan. Silakan coba lagi nanti.');
        }
    },
    dl: async function (link) {
        ...
        try {
            ...
            console.error(error);
            throw new Error('❌ Gagal membuat tautan unduhan. Layanan mungkin sedang sibuk.');
        }
    }
};
