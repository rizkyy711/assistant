...
const jadibot = async (rizkyyy, m, from) => {
  if (Object.keys(client).includes(from)) {
    return rizkyyy.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await rizkyyy.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../Rizkyy')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(rizkyyy, m, from) {
  if (!Object.keys(client).includes(from)) {
    return rizkyyy.sendMessage(from, {
...
async function listjadibot(rizkyyy, m) {
